...
begin
	...
	-- /*** DEBUG ***/
        -- DELETE FROM #ITEMS; INSERT INTO #ITEMS SELECT 44981
		-- SET @FILTER_ITEMS = 1
	...
	print 'поступления на склад'
		...
		group by acc_register_id, mfr_doc_id, mfr_number, item_id, unit_name, d_ship, id_ship, agent_id, invoice_id, has_invoice
    ...
    -- mark rows with bad mfr_number
    update #fifo set mfr_doc_id = -1 where isnull(mfr_number,'') != '' and mfr_doc_id = 0
    ...
    -- result
		if @debug = 1
			...
			select * from #fifo where id_job = 650802
		else
			select * from #fifo
