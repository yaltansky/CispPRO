...
begin
    ...
    -- #opers
        ...
        where 
            (@context = 'plans' 
                and c.mfr_doc_id in (select id from @docs)
                and isnull(x.d_to_fact, x.d_to_plan) between @d_from and @d_to
                )
            or (@context = 'contents'
                and c.content_id in (select id from @contents)
                )
            or (@context = 'opers'
                and x.oper_id in (select id from @opers)
                )

        create index ix_group on #opers(place_id, mfr_doc_id, product_id, item_id, resource_id, d_doc)

        insert into #opers(place_id, resource_id, loading_limit, work_type_name)
        select x.place_id, x.resource_id, rs.quantity * rs.loading, '-'
        from (
            select distinct place_id, resource_id from #opers
            ) x
    ...
    -- select
        ...
        from (
            ...
            join mfr_items_statuses st on st.status_id = r.oper_status_id
