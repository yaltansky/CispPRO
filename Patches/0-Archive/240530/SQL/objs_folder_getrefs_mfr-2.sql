...
begin
    ...
    -- Детали, материалы, состав изделия
        ...
        begin
            ...
            begin
                ...
                insert into objs_folders_details(folder_id, obj_type, obj_id, add_mol_id)
                select distinct @buffer_id, 'sd', r.id_ship, @mol_id
                from v_mfr_r_provides_all r
            ...
            begin
                exec objs_buffer_clear @mol_id, 'mftrf'

                insert into objs_folders_details(folder_id, obj_type, obj_id, add_mol_id)
                select distinct @buffer_id, 'mftrf', r.id_job, @mol_id
                from v_mfr_r_provides_all r
                    join @buffer buf on buf.id = r.id_mfr
            ...
            begin
                ...
                insert into objs_folders_details(folder_id, obj_type, obj_id, add_mol_id)
                select distinct @buffer_id, 'inv', prv.id_invoice, @mol_id
                from sdocs_mfr_contents x
                    join @buffer buf on buf.id = x.content_id
                    join v_mfr_r_provides_all prv on prv.id_mfr = x.content_id
                ...
                from mfr_sdocs_contents c
                    join @buffer buf on buf.id = c.content_id
                    join v_mfr_r_provides_all prv on prv.id_mfr = c.content_id
                        join supply_r_invpays_totals x on 
                                x.inv_id = prv.id_invoice
                            and x.mfr_doc_id = prv.mfr_doc_id
                            and x.item_id = prv.item_id
                where not exists(
                    select 1 from objs_folders_details where folder_id = @buffer_id and obj_type = 'invpay'
                    and obj_id = x.row_id
                    )
        end
    ...
    -- Библиотека
        ...
        begin
            exec objs_buffer_clear @mol_id, @obj_type_target
    ...
    -- Счета поставщиков
        ...
        begin
            ...
            begin            
                ...
                insert into objs_folders_details(folder_id, obj_type, obj_id, add_mol_id)
                select distinct @buffer_id, 'sd', x.id_ship, @mol_id
                from v_mfr_r_provides_all x
                    join @buffer i on i.id = x.id_invoice
