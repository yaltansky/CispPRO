...
begin
	...
	-- prepare sql
		...
		declare @where nvarchar(max) = concat(
			...
			, case 
				when @status_id is not null then 
					case
						...
						when @work_type_id in (1,3) then
							case
								when @status_id in (2) then
									' and exists(
								...
								when @status_id = -100 then 
									' and exists(
										...
										where content_id = x.content_id 
											and (@place_id is null or place_id = @place_id)
											and (status_id between -2 and 99)
											and (work_type_id = @work_type_id)
									)'
								when @status_id = 100 and @place_id is not null then 
									'' -- определяется @place_id (см.ниже)
								else
			...
			, case when @milestone_id is not null then concat(' and exists(
						select 1 from sdocs_mfr_opers with(nolock) where content_id = x.content_id and milestone_id = ', @milestone_id, ')'
						)
			end

			, case when @material_id is not null then concat(' and exists(
            ...
            , case when @resource_id is not null then concat(' and exists(
						select 1 from mfr_drafts_opers_resources with(nolock) where draft_id = x.draft_id and resource_id = ', @resource_id, ')'
						)
			end
			
            , case
				when @attr_name is not null then
					' and exists(select 1 from sdocs_mfr_contents_attrs with(nolock) where content_id = x.content_id and attr_id in (select id from @attrs))'
			end
		...
		declare @join nvarchar(max) = N''
			...
			+ case when exists(select 1 from #ids) then ' join #ids i on i.id = x.content_id ' else '' end
