	...
	-- fill #tables
		exec tracer_log @tid, '#pdm'
			...
			select
				...
				concat(h.PdmId, '-v', v.VariantNo),
				isnull(h.VersionNo, v.VariantNo),
		...
		exec tracer_log @tid, '#pdm_docs'
			insert into #pdm_docs(
				extern_pdm_id,
				version_number, number, name, url
				)
			select distinct
				concat(d.PdmId, '-v', d.VariantNo),
			    d.VersionNo, d.FileId, d.FileName, d.Url
        ...
        version_number, drawing_number, item_description, item_weight,
        ...
        --
        d.VersionNo, d.DrawingNo, d.Description, d.ItemWeight,
				-- 
	...
	BEGIN TRANSACTION
        ...
        exec tracer_log @tid, 'mfr_pdm_docs'
            ...
            insert into mfr_pdm_docs(
                pdm_id, version_number, number, name, url
                )
            select
                h.pdm_id, d.version_number, d.number, d.name, d.url
            from #pdm_docs d
                join #pdm h on h.extern_id = d.extern_pdm_id
        ...
        exec tracer_log @tid, 'mfr_pdm_items'
            ...
            select
                ...
                d.version_number, d.drawing_number, d.item_description, d.item_weight,
                unit_name, q_brutto, q_netto
            from #pdm_items d
                join #pdm h on h.extern_id = d.extern_pdm_id
